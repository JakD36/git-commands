...
fn main()
{
    ...
    let remote_branch = env::args().nth(2).expect("Need to provide a branch to mark as to be deleted!");
    ...
    let archived_branch_name = String::from("toBeDeleted/") + remote_branch_name;
    
    if remote_branch_name.to_lowercase().starts_with("toBeDeleted/")
    {
        eprintln!("This branch has already been marked as to be deleted");
        process::exit(1);
    }
    ...
    let output = process::Command::new(git_path)
        ...
        .arg(&remote)
        .arg(&archived_branch_name)
        .output().expect("Failed to push the new branch name");
    ...
    let output = process::Command::new(git_path)
        ...
        .arg("-u")
        .arg(&archived_branch_name)
        .output().expect("Failed to set upstream to the new branch");
    ...
    let output = process::Command::new(git_path)
        ...
        .arg("--delete")
        .arg(archived_branch_name)
        .output().expect("Failed to delete the local copy of the new branch");
